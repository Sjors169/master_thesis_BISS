...
# main function
def run_logistic_regression(X, y, output_dir="results/logistic_regression"):
    model_name = 'LogisticRegression'
    base_model = LogisticRegression(random_state=SEED, max_iter=3000)
    param_dist = [
    ...
    for scen_name, steps in scenarios.items():
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X, y)):
            ...
            feature_df = pd.DataFrame({
                ...
                'Normalized': np.abs(coefs) / np.sum(np.abs(coefs))
