...
# main function
def run_rf(X, y, output_dir="results/RandomForest"):
    model_name = 'RandomForest'
    base_model = RandomForestClassifier(random_state=SEED)

    param_dist = {
        'clf__n_estimators': IntDistribution(1, 2000),
        'clf__max_features': FloatDistribution(0.0, 1.0),
        'clf__min_weight_fraction_leaf': FloatDistribution(0.0, 0.5),
        'clf__max_samples': FloatDistribution(0.1, 1.0)
    }
    ...
    for scen_name, steps in scenarios.items():
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X, y)):
            ...
            fi_df['Normalized'] = fi_df['Importance'] / fi_df['Importance'].sum()
            fi_df.sort_values(by='Importance', ascending=False, inplace=True)
            fi_df.to_csv(
                os.path.join(output_dir, f"feature_importance_{model_name}_{scen_name}_fold{fold_idx}.csv"),
                index=False
            )
