...
target_col = 'conversion' if 'conversion' in df.columns else 'Conversion'
...
# Main function
def run_xgboost(X, y, output_dir="results/XGBoost"):
    model_name = 'XGBoost'
    base_model = XGBClassifier(random_state=SEED, eval_metric='logloss', use_label_encoder=False)

    param_dist = {
        'clf__n_estimators': IntDistribution(1, 5000),
        'clf__learning_rate': FloatDistribution(0.01, 0.3),
        'clf__gamma': FloatDistribution(0.0, 0.4),
        'clf__subsample': FloatDistribution(0.1, 1.0),
        'clf__max_depth': IntDistribution(1, 15),
        'clf__min_child_weight': IntDistribution(0, 7),
        'clf__colsample_bytree': FloatDistribution(0.0, 1.0),
        'clf__colsample_bylevel': FloatDistribution(0.0, 1.0),
        'clf__reg_lambda': FloatDistribution(0.0009765625, 1024),
        'clf__reg_alpha': FloatDistribution(0.0009765625, 1024)
    }
    ...
    for scen_name, steps in scenarios.items():
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X, y)):
            ...
            start = time.time()
            search = OptunaSearchCV(pipeline, param_distributions=param_dist, n_trials=100,
                                    cv=StratifiedKFold(5), scoring='f1', random_state=SEED, n_jobs=2)
            search.fit(X_train_scaled, y_train)
            best_pipe = search.best_estimator_
            best_params = search.best_params_
            elapsed = round(time.time() - start, 2)

            result = evaluate_model(f"{model_name}_{scen_name}_fold{fold_idx}", best_pipe,
                                    X_test_scaled, y_test, X_val_scaled, y_val)
            ...
            feature_df.to_csv(os.path.join(output_dir, f"{model_name}_{scen_name}_fold{fold_idx}_importance.csv"), index=False)
