...
# main functions
def run_nn(X, y, output_dir="correct/results/NeuralNetwork"):
    model_name = 'NeuralNetwork'
    base_model = MLPClassifier(random_state=SEED, max_iter=500, early_stopping=True)

    param_dist = {
        'clf__hidden_layer_sizes': CategoricalDistribution([(32,), (64,), (128,)]),
        'clf__activation': CategoricalDistribution(['relu']),
        'clf__alpha': FloatDistribution(1e-5, 1e-2, log=True),
        'clf__learning_rate_init': FloatDistribution(1e-4, 1e-2, log=True),
        'clf__batch_size': IntDistribution(64, 128)
    }
    ...
    for scen_name, steps in scenarios.items():
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X, y)):
            ...
            search = OptunaSearchCV(
                pipeline, param_distributions=param_dist, n_trials=25,
            ...
            result = evaluate_model(
                f"{model_name}_{scen_name}_fold{fold_idx}", best_pipe,
